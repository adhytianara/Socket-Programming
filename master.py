...
def main():
    ...
    while True:
        ...
        if user_action == 1:
            ...
            start_new_thread(check_job, ())
        elif user_action == 4:
            start_new_thread(check_job_execution, ())
        elif user_action == 0:
            print('Exit progam')
            exit()
...
def execute_jobs(job):
    ...
    job_execution[job] = selected_worker
...
def check_job_execution():
    ...
    if not job_execution:
        print()
        print('There is no job can be execute')
    else:
        print('\n')
        for key in job_execution:
            print('Job '+key+' got executed in Worker Node : '+job_execution[key])
    print('------------------------------------')
...
def check_job():
    for worker, status_job in workers.items():
        try:
            client = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            client.connect((worker, PORT))
            ...
            # Print Worker 'IP': 'Status'
            client.send(bytes('CHECK_STATUS_JOB','utf-8'))
            data = client.recv(1024).decode('utf-8')
            workers[worker] = data
            print(f'Worker {worker}, Status Job= : {data}')
...
def print_list_of_actions():
    ...
    action = input('\n{}\n{}\n{}\n{}\n{}\n{}\n{}\n{}\n{}'.format(
        ...
        "[3] Check worker status job",
        "[4] Check execution place",
        "[0] Exit",
        '************************',
